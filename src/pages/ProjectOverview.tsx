...
const ProjectOverview = () => {
  return (
    <div className="font-sans text-foreground bg-background">
      ...
      {/* Back button */}
      <div className="sticky top-0 z-50 bg-card/90 backdrop-blur-md border-b border-border px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-primary font-semibold">
          <Shield className="w-5 h-5" />
          AI-Based Vehicle Re-Identification System
        </div>
      ...
      {/* SLIDE 1 — Title */}
      <section className="min-h-screen flex flex-col justify-center px-12 py-16 bg-card/30 border-b border-border">
      ...
      <section className="min-h-screen flex flex-col justify-center px-12 py-16 border-b border-border">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-card border border-border rounded-xl p-6 space-y-3">
              <Eye className="w-7 h-7 text-destructive" />
              <h3 className="font-semibold text-lg text-foreground">Manual CCTV Review</h3>
              <p className="text-muted-foreground text-sm leading-relaxed">Officers spend hours manually scanning hundreds of CCTV feeds to find a single vehicle — slow, error-prone, and inefficient.</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-6 space-y-3">
              <Search className="w-7 h-7 text-yellow-500" />
              <h3 className="font-semibold text-lg text-foreground">Partial Information</h3>
              <p className="text-muted-foreground text-sm leading-relaxed">Witnesses provide incomplete details: partial plate numbers, rough color descriptions, uncertain brand/model guesses.</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-6 space-y-3">
              <Camera className="w-7 h-7 text-blue-500" />
              <h3 className="font-semibold text-lg text-foreground">Multi-Camera Challenge</h3>
              <p className="text-muted-foreground text-sm leading-relaxed">A vehicle moves across multiple cameras. Linking the same vehicle across feeds requires re-identification — a hard computer vision problem.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* SLIDE 3 — Solution */}
      <section className="min-h-screen flex flex-col justify-center px-12 py-16 bg-card/20 border-b border-border">
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="text-center space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                {["Witness Input Panel — collects crime scene details", "AI Processing View — animated pipeline progress display", "Vehicle Results Dashboard — CCTV match cards with images", "Vehicle Comparison View — side-by-side similarity analysis"].map(item => (
                  <li key={item} className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />{item}</li>
                ))}
              </ul>
            </div>
            <div className="space-y-4">
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                {["Lovable Cloud Edge Function — serverless, auto-scaling", "Gemini AI Gateway — simulates full YOLOv8 + CNN pipeline", "Python FastAPI reference — real YOLOv8/ResNet for GPU servers", "CORS-enabled REST API with structured tool-call responses"].map(item => (
                  <li key={item} className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />{item}</li>
                ))}
              </ul>
            </div>
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="grid md:grid-cols-4 gap-5">
            {[
              ...
              { stage: "Stage 3", title: "Color + OCR", icon: <Eye className="w-8 h-8 text-yellow-500" />, points: ["HSV color space analysis", "14 color classes supported", "Partial plate OCR matching", "Wildcard pattern support"] },
            ...
            <div className="bg-card border border-border rounded-xl p-6 space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { field: "Vehicle Color ⭐", eg: "White, Black, Red, Blue, Silver, Gray…" },
                  { field: "Vehicle Brand ⭐", eg: "Toyota, Honda, BMW, Hyundai, Audi…" },
              ...
              <div className="bg-primary/10 border border-primary/30 rounded-lg p-3 text-xs text-primary">
                ⭐ Color and Brand are the most critical inputs — they directly drive the AI matching confidence score.
              </div>
            </div>
            <div className="bg-card border border-border rounded-xl p-6 space-y-4">
      ...
      <section className="min-h-screen flex flex-col justify-center px-12 py-16 border-b border-border">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { cat: "Frontend", colorClass: "text-blue-500", items: ["React 18 + TypeScript", "Vite (build tool)", "Tailwind CSS (styling)", "shadcn/ui components", "Lucide React icons"] },
              { cat: "AI / Backend", colorClass: "text-purple-500", items: ["Gemini 3 Flash (AI Gateway)", "Lovable Cloud Edge Functions", "Python FastAPI (reference)", "YOLOv8 (Ultralytics)", "ResNet-50 / MobileNetV2"] },
              { cat: "Data & Infra", colorClass: "text-green-500", items: ["Lovable Cloud (hosted backend)", "5 Vehicle Datasets (378K+ images)", "scikit-learn cosine similarity", "OpenCV HSV color detection", "Supabase JS SDK"] },
            ].map(col => (
              <div key={col.cat} className="bg-card border border-border rounded-xl p-5 space-y-3">
                <h3 className={`font-bold text-lg ${col.colorClass}`}>{col.cat}</h3>
      ...
      <section className="min-h-screen flex flex-col justify-center px-12 py-16 bg-card/20 border-b border-border">
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="text-center space-y-3">
            ...
            <p className="text-muted-foreground">Input: White Toyota Innova, Partial plate KA-05-**-1234, MG Road, 18:00–19:00</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              ...
              <div key={r.cam} className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${r.status === "EXACT" ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}>
                      {r.conf}% {r.status}
                    </span>
                  </div>
      ...
      {/* SLIDE 10 — Conclusion */}
      <section className="min-h-screen flex flex-col justify-center px-12 py-16">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-5 text-left">
            {[
              ...
              "Color & Brand matching are primary confidence boosters in the algorithm",
            ].map(point => (
